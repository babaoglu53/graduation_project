...
void get_door_state(){
  if(WiFi.status()== WL_CONNECTED){
      ...
      String serverPath = serverName + "&sheet=giris&api_key=";
      ...
      if (httpResponseCode>0) {
        ...
        JsonArray& rowsArray = root["columns"]["kapi_durum"];
        int rows_array_size = rowsArray.size();
        Serial.println(F("Response:"));
        String kapi_durum = root["columns"]["kapi_durum"][rows_array_size-1].as<String>();
        Serial.println("Kapi Durum: " + kapi_durum);
        if(kapi_durum.toInt())
        {
